...
contract DeployScript is Script {
    ...
    function run() public {
        ...
        console.log("========================================");
        console.log("");
        console.log("The Market contract needs NFT addresses set via 3-of-3 MultiSig.");
        console.log("Run the following after this deployment completes:");
        console.log("");
        console.log("Option A: Use the SetupMultiSig script:");
        console.log("  forge script script/SetupMultiSig.s.sol:SetupMultiSigScript --rpc-url $BNB_TESTNET_RPC --broadcast");
        console.log("");
        console.log("Option B: Manual steps (each signer must execute):");
        console.log("");
        console.log("1. Signer 1 proposes and auto-approves:");
        console.log("   market.proposeUpdatePredictorAccessPass(", address(accessPass), ")");
        ...
        console.log("   market.proposeUpdateSignalKeyNFT(", address(signalKey), ")");
        console.log("5. Signer 2 approves the action");
        console.log("6. Signer 3 approves (auto-executes)");
        console.log("");
        console.log("After Phase 2, verify: market.isFullyInitialized() == true");
        console.log("");
        console.log("========================================");
     ...
     */
    function _writeDeploymentFile(address treasury, address[3] memory signers) internal {
        string memory line1 = string(abi.encodePacked(
            "# SignalFriend Deployment - Chain ID: ", vm.toString(block.chainid), "\n",
            "# Deployed at block: ", vm.toString(block.number), "\n\n"
        ));
        
        string memory line2 = DEPLOY_MOCK_USDT 
            ? string(abi.encodePacked("MOCK_USDT=", vm.toString(address(mockUsdt)), "\n"))
            : "";
            
        string memory line3 = string(abi.encodePacked(
            "SIGNAL_FRIEND_MARKET=", vm.toString(address(market)), "\n",
            "PREDICTOR_ACCESS_PASS=", vm.toString(address(accessPass)), "\n",
            "SIGNAL_KEY_NFT=", vm.toString(address(signalKey)), "\n"
        ));
        
        string memory line4 = string(abi.encodePacked(
            "PLATFORM_TREASURY=", vm.toString(treasury), "\n",
            "MULTISIG_SIGNER_1=", vm.toString(signers[0]), "\n",
            "MULTISIG_SIGNER_2=", vm.toString(signers[1]), "\n",
            "MULTISIG_SIGNER_3=", vm.toString(signers[2]), "\n"
        ));
        
        string memory deploymentInfo = string(abi.encodePacked(line1, line2, line3, line4));
