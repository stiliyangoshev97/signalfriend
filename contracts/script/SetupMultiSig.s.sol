...
contract SetupMultiSigScript is Script {
    function run() public {
        ...
        require(
            signerRole >= 1 && signerRole <= 3,
            "SIGNER_ROLE must be 1, 2, or 3"
        );
        ...
        if (signerRole == 1) {
            ...
            if (
                actionId1 != bytes32(0) &&
                market.predictorAccessPass() == address(0)
            ) {
                console.log("");
                console.log("Approving Action 1:", vm.toString(actionId1));
                market.approveAction(actionId1);
                console.log(
                    "  Approved! Current approvals:",
                    market.getActionApprovals(actionId1)
                );

                if (market.isActionExecuted(actionId1)) {
                    console.log(
                        "  ACTION EXECUTED! PredictorAccessPass is now set."
                    );
                }
            }
            ...
            if (
                actionId2 != bytes32(0) && market.signalKeyNFT() == address(0)
            ) {
                console.log("");
                ...
                console.log(
                    "  Approved! Current approvals:",
                    market.getActionApprovals(actionId2)
                );
